...
# Coverage-rule version. 2.0 adds the absolute criterion floor: a
# determinant-year resting on fewer than `min_criteria` criteria is flagged
# regardless of its sub-factor ratio. Closes the 1-of-1 loophole (a
# single-sub-factor determinant always passed the ratio check).
COVERAGE_RULE_VERSION = "2.0"
...
def compute_scores(df: pd.DataFrame,
                   ...
                   min_criteria: int = 2) -> pd.DataFrame:
    ...
    has_sub = "sub_factor" in d.columns      # two-level (IPS) hierarchy present?
    ...
    for key_vals, g in d.groupby(norm_keys):
        ...
        sub = g["sub_factor"].iloc[0] if has_sub else None
        ...
        for k, s in scored.items():
            ...
            rows.append((country, year, gkey["factor"], gkey["context"],
                         gkey["determinant"], sub, gkey["indicator"], s))

    norm = pd.DataFrame(rows, columns=["country", "year", "factor", "context",
                                       "determinant", "sub_factor", "indicator", "index"])
    if not has_year:
        norm = norm.drop(columns=["year"])

    det_grp = ["country", "factor", "context", "determinant"]
    if has_year:
        det_grp = ["country", "year", "factor", "context", "determinant"]

    if has_sub:
        # 2a) indicators -> sub-factor (equal weight per criterion within a sub-factor)
        sub_scores = (norm.groupby(det_grp + ["sub_factor"])
                          .agg(index=("index", "mean")).reset_index())
        # 2b) sub-factors -> determinant (equal weight per sub-factor: a data-rich
        #     sub-factor cannot outweigh a sparse one). INDEX UNCHANGED from before.
        scores = (sub_scores.groupby(det_grp)
                            .agg(index=("index", "mean")).reset_index())
        # total criteria feeding each determinant (for the coverage report)
        n_ind = (norm.groupby(det_grp)
                     .agg(n_indicators=("indicator", "nunique")).reset_index())
        scores = scores.merge(n_ind, on=det_grp)

        # --- partial-coverage rule (additive; does not change `index`) -----------
        # present: distinct sub-factors that actually fed THIS determinant cell.
        present = (sub_scores.groupby(det_grp)["sub_factor"].nunique()
                             .rename("n_sub_factors_present").reset_index())
        # expected: sub-factors the framework defines for (factor, context,
        # determinant) -- i.e. all sub-factors realized anywhere in this run.
        fcd = ["factor", "context", "determinant"]
        expected = (norm.groupby(fcd)["sub_factor"].nunique()
                        .rename("n_sub_factors_expected").reset_index())
        scores = scores.merge(present, on=det_grp).merge(expected, on=fcd)
        ratio = scores["n_sub_factors_present"] / scores["n_sub_factors_expected"]
        # Two-part rule, flag (don't drop) a corner that is either:
        #  (a) resting on < coverage_min_ratio of its sub-factors, or
        #  (b) resting on fewer than min_criteria criteria in absolute terms.
        # (b) is what catches single-sub-factor determinants: 1-of-1 always
        # passes (a), but one criterion is not a determinant.
        scores["coverage_suppressed"] = ((ratio < coverage_min_ratio) |
                                         (scores["n_indicators"] < min_criteria))
    else:
        # single-level (original behaviour; keeps the published validation exact)
        scores = (norm.groupby(det_grp)
                      .agg(index=("index", "mean"),
                           n_indicators=("indicator", "nunique")).reset_index())
        # coverage columns are not applicable without sub-factors
        scores["n_sub_factors_present"] = pd.NA
        scores["n_sub_factors_expected"] = pd.NA
        scores["coverage_suppressed"] = False
    return scores
