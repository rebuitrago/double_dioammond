...
# ---- WTO trade relay (fills the 2025 international corners) ----------------
# Gap-fills 2025 Exports %GDP and Trade %GDP from the WTO Timeseries API, onto
# the WDI goods+services criteria below. Needs WTO_API_KEY in the environment;
# if it's unset the loader simply skips (the rest of the load is unaffected).
# Construction (validated against WDI 2022-24, ~0.3pp agreement on Brazil):
#   Exports %GDP = (goods_exp + services_exp) x 1e6 / GDP_2025 x 100
#   Trade   %GDP = (above + goods_imp + services_imp) x 1e6 / GDP_2025 x 100
#   GDP_2025 (nominal) = GDP_2024(NY.GDP.MKTP.CD)
#                        x (1 + real_growth_2025/100) x (1 + inflation_2025/100)
# 2025 rows are tagged 'estimate'. Coverage is PARTIAL by design — only countries
# that have WTO trade AND 2024 GDP AND 2025 growth AND 2025 inflation get a value.
WTO_API_BASE  = "https://api.wto.org/timeseries/v1"
WTO_KEY_ENV   = "WTO_API_KEY"
WTO_YEARS     = "2024-2025"            # 2024 = built-in validation; 2025 = relay target
WTO_VINTAGE   = "WTO 2026 + derived GDP"
# (indicator code, product/sector code kept as the 'total')
WTO_GOODS_EXP = ("ITS_MTV_AX", "TO")   # total merchandise exports
WTO_GOODS_IMP = ("ITS_MTV_AM", "TO")   # total merchandise imports
WTO_SVC_EXP   = ("ITS_CS_QAX", "S")    # total services exports  (S = 'Total services')
WTO_SVC_IMP   = ("ITS_CS_QAM", "S")    # total services imports
WTO_TARGET_EXPORTS = "NE.EXP.GNFS.ZS"      # International demand (Exports %GDP)
WTO_TARGET_TRADE   = "NE.TRD.GNFS.ZS"      # Market openness      (Trade %GDP)
WTO_GDP_CODE       = "NY.GDP.MKTP.CD"      # 2024 nominal GDP, current US$
WTO_GROWTH_CODE    = "NY.GDP.MKTP.KD.ZG"   # 2025 real growth %  (relayed Euromonitor)
WTO_INFLATION_CODE = "EUROMONITOR.INFLATION"  # 2025 inflation %
...
def _wto_get(path, **params):
    """One GET against the WTO Timeseries API (raises on HTTP error)."""
    import requests
    params["subscription-key"] = os.environ.get(WTO_KEY_ENV)
    r = requests.get(f"{WTO_API_BASE}/{path}", params=params, timeout=90)
    r.raise_for_status()
    return r.json()


def _wto_reporters_iso3(valid):
    """{WTO numeric reporter code -> ISO3} for panel economies (aggregates dropped)."""
    out = {}
    for e in _wto_get("reporters", lang="1"):
        iso = e.get("iso3A")                 # null for World/regions -> skipped
        if iso and iso in valid:
            out[str(e.get("code"))] = iso
    return out


def _wto_fetch(indicator, sector, code2iso, years):
    """{(iso3, year) -> value_$M} for one indicator, filtered to its total-sector code."""
    out, off = {}, 0
    while True:
        data = _wto_get("data", i=indicator, r="all", ps=years, pc=sector,
                        fmt="json", mode="full", max="100000", off=str(off),
                        lang="1", meta="false")
        rows = data.get("Dataset") or []
        for row in rows:
            if row.get("ProductOrSectorCode") != sector:
                continue                      # belt-and-suspenders: pc filter + check
            iso = code2iso.get(str(row.get("ReportingEconomyCode")))
            v = row.get("Value")
            if iso and v is not None:
                out[(iso, int(row["Year"]))] = float(v)
        if len(rows) < 100000:
            break
        off += 100000
    return out
...
def _obs_year(client, indicator_id, year):
    """{iso3 -> raw_value} for one indicator in one year (sees the relayed value too)."""
    if indicator_id is None:
        return {}
    out, start = {}, 0
    while True:
        resp = (client.table("observation").select("country_iso3,raw_value")
                .eq("indicator_id", indicator_id).eq("year", year)
                .range(start, start + 999).execute())
        b = resp.data or []
        for r in b:
            out[r["country_iso3"]] = float(r["raw_value"])
        if len(b) < 1000:
            break
        start += 1000
    return out
...
def load_wto_trade(client, code_to_id, valid) -> int:
    """Relay WTO goods+services trade into 2025 Exports %GDP and Trade %GDP."""
    if not os.environ.get(WTO_KEY_ENV):
        print(f"  ! WTO: {WTO_KEY_ENV} not set — skipped (export your key to enable it). "
              f"Rest of load unaffected.")
        return 0
    exp_id = code_to_id.get(WTO_TARGET_EXPORTS)
    trd_id = code_to_id.get(WTO_TARGET_TRADE)
    if not exp_id or not trd_id:
        print(f"  ! WTO: targets {WTO_TARGET_EXPORTS}/{WTO_TARGET_TRADE} not in catalog. skipped")
        return 0
    try:
        code2iso = _wto_reporters_iso3(valid)
        ge = _wto_fetch(*WTO_GOODS_EXP, code2iso, WTO_YEARS)
        gi = _wto_fetch(*WTO_GOODS_IMP, code2iso, WTO_YEARS)
        se = _wto_fetch(*WTO_SVC_EXP, code2iso, WTO_YEARS)
        si = _wto_fetch(*WTO_SVC_IMP, code2iso, WTO_YEARS)
    except Exception as e:
        print(f"  ! WTO: fetch failed ({type(e).__name__}: {e}). skipped — check key/network. "
              f"Rest of load unaffected.")
        return 0
    if not ge:
        print("  ! WTO: no merchandise-export rows returned (unexpected). skipped")
        return 0

    gdp24  = _obs_year(client, code_to_id.get(WTO_GDP_CODE), 2024)
    grow25 = _obs_year(client, code_to_id.get(WTO_GROWTH_CODE), 2025)
    infl25 = _obs_year(client, code_to_id.get(WTO_INFLATION_CODE), 2025)

    # --- built-in validation on 2024 (ground truth) --------------------------
    # WTO goods+services over ACTUAL 2024 GDP, vs WDI's own measured 2024 ratio.
    wdi_exp24 = _obs_year(client, exp_id, 2024)
    diffs = []
    for iso in {i for (i, y) in ge if y == 2024}:
        if (iso, 2024) in se and iso in gdp24 and iso in wdi_exp24:
            wto_exp = (ge[(iso, 2024)] + se[(iso, 2024)]) * 1e6 / gdp24[iso] * 100.0
            diffs.append(abs(wto_exp - wdi_exp24[iso]))
    if diffs:
        diffs.sort()
        med = diffs[len(diffs) // 2]
        print(f"    WTO 2024 check: exports%GDP vs WDI actual — median |Δ| {med:.2f}pp "
              f"over {len(diffs)} countries (small = construction matches WDI)")

    # --- 2025 relay ----------------------------------------------------------
    exp_front = _primary_frontier(client, exp_id)
    trd_front = _primary_frontier(client, trd_id)
    isos = ({i for (i, y) in ge if y == 2025} & {i for (i, y) in gi if y == 2025}
            & {i for (i, y) in se if y == 2025} & {i for (i, y) in si if y == 2025})
    rows, covered, skipped = [], set(), 0
    for iso in sorted(isos):
        g, gr, inf = gdp24.get(iso), grow25.get(iso), infl25.get(iso)
        if g is None or gr is None or inf is None:
            skipped += 1
            continue
        gdp25 = g * (1 + gr / 100.0) * (1 + inf / 100.0)
        exp_usd = (ge[(iso, 2025)] + se[(iso, 2025)]) * 1e6
        imp_usd = (gi[(iso, 2025)] + si[(iso, 2025)]) * 1e6
        exports_pct = exp_usd / gdp25 * 100.0
        trade_pct   = (exp_usd + imp_usd) / gdp25 * 100.0
        if exp_front is None or 2025 > exp_front:
            rows.append({"indicator_id": exp_id, "country_iso3": iso, "year": 2025,
                         "raw_value": exports_pct,
                         "source_vintage": WTO_VINTAGE + RELAY_VINTAGE_SUFFIX,
                         "value_type": "estimate"})
        if trd_front is None or 2025 > trd_front:
            rows.append({"indicator_id": trd_id, "country_iso3": iso, "year": 2025,
                         "raw_value": trade_pct,
                         "source_vintage": WTO_VINTAGE + RELAY_VINTAGE_SUFFIX,
                         "value_type": "estimate"})
        covered.add(iso)
    rows = _dedupe(rows)
    n = _upsert(rows)
    print(f"  + WTO trade relay: {n} rows  [2025 Exports%GDP + Trade%GDP]  "
          f"{len(covered)} countries with full inputs; "
          f"{skipped} dropped (missing 2024 GDP / 2025 growth / 2025 inflation)")
    return n
...
def main() -> None:
    ...
    total += load_wto_trade(client, code_to_id, valid)
