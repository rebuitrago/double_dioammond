...
from ddd.connectors import fetch_wdi, fetch_wgi, attach_indicator_ids, upsert_observations
...
def ingest(start: int, end: int, vintage: str | None = None) -> int:
    ...
    wgi_codes = [c for c in codes if c.endswith(".EST")]
    wdi_codes = [c for c in codes if not c.endswith(".EST")]
    print(f"Fetching {len(wdi_codes)} WDI + {len(wgi_codes)} WGI indicators x {len(countries)} countries, {start}-{end}...")
    frames = []
    if wdi_codes:
        frames.append(fetch_wdi(wdi_codes, countries, start, end, db=2))
    if wgi_codes:
        frames.append(fetch_wgi(wgi_codes, countries, start, end))
