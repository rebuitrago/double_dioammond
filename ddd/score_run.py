...
def build_scoring_frame(observations: pd.DataFrame,
                        ...
                        sub_factors: pd.DataFrame | None = None) -> pd.DataFrame:
    ...
    indicators   : id, determinant_id, context, polarity, method, active
                   (and sub_factor_id when the two-level hierarchy is in use)
    determinants : id, factor_id, name
    factors      : id, name
    sub_factors  : id, determinant_id, name  (optional; enables IPS two-level scoring)
    """
    ...
    use_sub = (sub_factors is not None and not sub_factors.empty
               and "sub_factor_id" in ind.columns)
    if use_sub:
        sf = sub_factors.rename(columns={"id": "sub_factor_id", "name": "sub_factor"})
        ind = ind.merge(sf[["sub_factor_id", "sub_factor"]], on="sub_factor_id", how="left")
    ...
    if use_sub:
        long["sub_factor"] = long["sub_factor"].fillna("General")
        cols.append("sub_factor")
    return long[cols]
...
def score_run(label: str,
    ...
    # sub_factor table may not exist before the migration; degrade gracefully
    try:
        sub_factors = _read_all(client, "sub_factor", "id,determinant_id,name")
    except Exception:
        sub_factors = pd.DataFrame()
    ind_cols = "id,determinant_id,context,polarity,method,active"
    if not sub_factors.empty:
        ind_cols = "id,determinant_id,sub_factor_id,context,polarity,method,active"
    ind_filter = {"id": indicator_ids} if indicator_ids else None
    indicators = _read_all(client, "indicator", ind_cols, ind_filter)
    ...
    frame = build_scoring_frame(observations, indicators, determinants, factors, sub_factors)
    scores = compute_scores(frame, normalization=normalization,
                            ...
                            min_criteria=coverage_min_criteria)
    scores = attach_determinant_ids(scores, frame)
    cov = coverage_report(frame, scores)

    # suppression breakdown: which part of the two-part rule fired
    by_ratio = ((scores["n_sub_factors_present"] / scores["n_sub_factors_expected"])
                < coverage_min_ratio)
    by_floor = scores["n_indicators"] < coverage_min_criteria
    supp = scores[scores["coverage_suppressed"] == True]  # noqa: E712
    latest = int(scores["year"].max()) if "year" in scores.columns else None
    supp_detail = None
    if latest is not None and not supp.empty:
        supp_detail = (supp[supp["year"] == latest]
                       .groupby(["factor", "context", "determinant"])
                       .size().rename("countries_suppressed").reset_index()
                       .sort_values("countries_suppressed", ascending=False))
    ...
    summary = {
        ...
        "n_suppressed": int(scores["coverage_suppressed"].sum()),
        "n_suppressed_by_ratio": int((by_ratio & scores["coverage_suppressed"]).sum()),
        "n_suppressed_by_floor": int((by_floor & scores["coverage_suppressed"]).sum()),
        "suppression_latest_year": latest,
        "suppression_detail": supp_detail,
        "coverage": cov,
    }
    if dry_run:
        summary["run_id"] = None
    ...
    payload = [{
        ...
        "n_sub_factors_present": (int(r.n_sub_factors_present)
                                  if pd.notna(r.n_sub_factors_present) else None),
        "n_sub_factors_expected": (int(r.n_sub_factors_expected)
                                   if pd.notna(r.n_sub_factors_expected) else None),
        "coverage_suppressed": bool(r.coverage_suppressed),
...
def _print_summary(s: dict) -> None:
    ...
    if s.get("n_suppressed"):
        print(f"  suppressed: {s['n_suppressed']} determinant cells flagged below the coverage rule "
              f"(ratio: {s.get('n_suppressed_by_ratio', '?')}, "
              f"criterion floor: {s.get('n_suppressed_by_floor', '?')}; "
              f"a cell can trip both). Flagged, not dropped.")
    detail = s.get("suppression_detail")
    if detail is not None and not detail.empty:
        print(f"\nFlagged cells in {s['suppression_latest_year']} "
              f"(countries per determinant):")
...
if __name__ == "__main__":
    ...
    p.add_argument("--coverage-min", type=float, default=0.5,
                   help="suppress a determinant cell resting on fewer than this "
                        "fraction of its sub-factors (default 0.5)")
    p.add_argument("--coverage-min-criteria", type=int, default=2,
                   help="flag a determinant cell resting on fewer than this many "
                        "criteria in absolute terms (default 2; closes the "
                        "single-sub-factor loophole)")
    ...
    _print_summary(score_run(
        ...
        coverage_min_ratio=a.coverage_min,
        coverage_min_criteria=a.coverage_min_criteria,
        dry_run=a.dry_run))
