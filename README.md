...
`run`/`score`, and a `diamond_coords` view. Then run `db/migration_01_subfactors.sql` to add the sub-factor layer
(factor -> determinant -> sub-factor -> indicator), the IPS two-level
hierarchy. The seed loads a starter indicator
